...
class SumPolynomialList(unittest.TestCase):
    ...
    def test_example(self):
        ...
        self.assertTrue(ring.isograde(p_sum, 0) == p0)
        self.assertTrue(ring.isograde(p_sum, 1) == p1)
        self.assertTrue(ring.isograde(p_sum, 2) == p2)
        self.assertTrue(ring.isograde(p_sum, 3) == p3)
...
class DbKeys(unittest.TestCase):
    ...
    def test_one(self):
        self.assertEqual(LieTriangle._make_db_key(0, 1), '(0,1)')
        
    def test_another(self):
        self.assertEqual(LieTriangle._make_db_key(100,99), '(100,99)')
...
class Binomial(unittest.TestCase):
    ...
    def test_zero_choose_zero(self):
        self.assertEqual(self._choose(0, 0), 1) #?check?
    ...
    def test_values(self):
        examples = {(0, 0): 1,
                    ...
                    (4, 2): 6,
                    (9, 8): 9}
        for (n, k), b in list(examples.items()):
            self.assertEqual(self._choose(n, k), b)

    def test_identity0(self):
        for n in range(10):
            lhs = self._choose(n, 0)
            rhs = self._choose(n, n)
            self.assertTrue(lhs == rhs == 1)
    ...
    def test_cf_implementations(self):
        for i in range(1, 10):
            for j in range(1, i+1):
                ...
                b1 = self._choose(i, j)
                self.assertEqual(b0, b1, (i,j,b0,b1))
...
class SolveHomologicalEquation(BlankLieTriangle):
    ...
    def test_powers_differing_by_unity_induce_sum(self):
        ...
        for mon in [Polynomial.Monomial((1, 2, 3, 4, 5, 6)),
            ...
            self.assertTrue((gen - expected).l1_norm() < 1.0e-15, (gen, expected))
...
class HillBase:
    ...
    def hill_about_l1(self):
        ...
        self.assertTrue(h_2 == alg.isograde(h, 2))
        return h
    ...
    def diagonalize(self, dof, h):
        ...
        self.assertTrue(lie.is_diagonal_polynomial(h_comp.homogeneous(2)))
...
class NormalizeAlreadyNormalized(HillBase, unittest.TestCase):
    ...
    def test_hill_quadratic_invariant(self):
        ...
        for i in range(steps):
            ...
            if i == 0:
                self.assertTrue(k_term == self.alg.isograde(h_dc, 2))
            else:
                self.assertTrue(k_term == self.alg.zero())
            h_dict, w_list, row = nf.get_state()
            self.assertTrue(w_list[i] == self.alg.zero())
...
class NormalizeHillThreeDof(HillBase, unittest.TestCase):
    ...
    def test_hill_cubic(self):
        ...
        self.assertTrue(k_real.imag().l_infinity_norm() < 1.0e-14)
        k_real = k_real.real()
        #print k_real
    ...
    def test_hill_cubic_cf_in_steps(self):
        ...
        for i in range(0, last_step + 1):
            h_term = self.alg.isograde(h_dc, i + 2)
            k_term, w_term = nf.compute_normal_form_and_generating_function(h_term)
            self.assertTrue(self.alg.is_isograde(k_term, i + 2))
            self.assertTrue(self.alg.is_isograde(w_term, i + 2))
            if i == 0:
                self.assertTrue(k_term == h_dc_2)
            k_poly += k_term
        h_dict, w_list, row = nf.get_state()
        self.assertTrue(row == last_step)
        ...
        self.assertTrue(steps_row == last_step)

        self.assertTrue(h_dict == steps_h_dict)
        self.assertTrue(w_list == steps_w_list)

        #compare the two methods:
        l1_tol = 1.0e-15
        for i in range(last_step + 1):
            x, y = (w_list[i], steps_w_list[i])
            self.assertTrue(self.alg.grade(x) == self.alg.grade(y))
            err = (x - y).l1_norm()
            self.assertTrue(err < l1_tol, err)
