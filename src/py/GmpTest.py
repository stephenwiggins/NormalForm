...
def equal_to_bits_prec(a, b, bits):
    return abs(a-b) < 1.0/float(1<<bits)
...
class BasicFloats(unittest.TestCase):
    ...
    def test_make_one_plus_epsilon(self):
        ...
        self.assertTrue(one != eps)
        self.assertTrue(eps > 0.0)
        self.assertTrue(one+eps > 1.0)
        self.assertTrue(float(one+eps) == 1.0)
    ...
    def test_addition(self):
        ...
        self.assertTrue(mpz(1.0/((one+eps)-one)) == mpz(10)**38)

    def test_numbers_are_immutable_in_present_version(self):
        one = mpf('1.0', 32)
        one_orig = one
        one += 1.0
        self.assertTrue(one == 2.0)
        self.assertTrue(one_orig == 1.0)
    ...
    def test_can_pickle_string_and_retrieve(self):
        ...
        def pkl_to_mpf(xxx_todo_changeme):
            (digit_str, prec) = xxx_todo_changeme
            return mpf(digit_str, prec, 10)
        s = pickle.dumps(mpf_to_pkl(num))
        num_new = pkl_to_mpf(pickle.loads(s))
        self.assertTrue(equal_to_bits_prec(num, num_new, bits))
    ...
    def test_equal_to_bits_prec(self):
        ...
        for i in range(bits*2):
            if i <= bits:
                self.assertTrue(equal_to_bits_prec(1.0, num, i))
            else:
                self.assertTrue(not equal_to_bits_prec(1.0, num, i))
